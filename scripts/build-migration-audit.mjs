 ...
 * and `tests/unit/migration-audit-generation.test.ts` hits it in CI.
 */
...
-- Regenerate with \`npm run db:audit\`; \`npm run check:migration-audit\` and the
-- unit suite both fail when this file and supabase/migrations/ disagree.
--
...
function main() {
  ...
  if (process.argv.includes('--check')) {
    let current;
    try {
      current = readFileSync(AUDIT_SQL_PATH, 'utf8');
    } catch {
      console.error(`${AUDIT_SQL_PATH} is missing. Run \`npm run db:audit\`.`);
      process.exit(1);
      return;
    }
    if (current !== sql) {
      console.error(
        'scripts/sql/audit-migrations-against-database.sql is out of date. Run `npm run db:audit`.',
      );
      process.exit(1);
      return;
    }
    console.log(`Migration audit is up to date (${migrationNames().length} migrations).`);
    return;
  }
