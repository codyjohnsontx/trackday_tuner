...
with expected(ordinality, migration, object_kind, object_name, present) as (values
  (1,  '20260223000000_init_baseline_schema',            'table',    'public.sessions',
       to_regclass('public.sessions') is not null),
  (2,  '20260224000000_add_sag_entries',                 'table',    'public.sag_entries',
       to_regclass('public.sag_entries') is not null),
  (3,  '20260224000100_add_stripe_profile_fields',       'column',   'public.profiles.stripe_customer_id',
       exists (select 1 from information_schema.columns
               where table_schema='public' and table_name='profiles'
                 and column_name='stripe_customer_id')),
  (4,  '20260228000200_add_session_module_metadata',     'column',   'public.sessions.enabled_modules',
       exists (select 1 from information_schema.columns
               where table_schema='public' and table_name='sessions'
                 and column_name='enabled_modules')),
  (5,  '20260420000300_add_ai_requests',                 'table',    'public.ai_requests',
       to_regclass('public.ai_requests') is not null),
  (6,  '20260422000400_add_adaptive_race_engineer',      'table',    'public.ai_recommendations',
       to_regclass('public.ai_recommendations') is not null),
  (7,  '20260424000500_add_ai_request_observability',    'index',    'ai_requests_user_fingerprint_created_idx',
       to_regclass('public.ai_requests_user_fingerprint_created_idx') is not null),
  (8,  '20260626000600_add_vehicle_baselines',           'table',    'public.vehicle_baselines',
       to_regclass('public.vehicle_baselines') is not null),
  (9,  '20260705000700_add_session_changes',             'table',    'public.session_changes',
       to_regclass('public.session_changes') is not null),
  -- The reported defect. Nine arguments, in the order the route sends them.
  (10, '20260716000800_add_session_outcomes',            'function', 'public.save_session_outcome(uuid,uuid,uuid,uuid,text,smallint,text[],text,smallint)',
       to_regprocedure('public.save_session_outcome(uuid,uuid,uuid,uuid,text,smallint,text[],text,smallint)') is not null),
  (11, '20260717000900_add_session_laps',                'table',    'public.session_laps',
  ...
  (15, '20260824001300_add_vehicle_photos_storage_policies','policy', 'vehicle-photos: insert own (storage.objects)',
  ...
  (16, '20260901001400_guard_replace_session_laps',      'function', 'public.replace_session_laps(uuid,uuid,jsonb,integer) OR its 1500 replacement',
  ...
  (17, '20260903001500_replace_session_laps_compares_lap_content','function','public.session_laps_identity(jsonb)',
