 ...
 * bike's baseline and Race Engineer memory. On the free plan that is also the
...
export function describeVehicleDeletion(nickname: string, counts: VehicleDeletionCounts): string {
  const baseline = counts.hasBaseline ? ` and the baseline setup you saved for it` : '';
  if (counts.sessionCount === 0) {
    return counts.hasBaseline
      ? `Deleting ${nickname} removes it from your garage${baseline}. No sessions are logged on it. This cannot be undone.`
  ...
  return `Deleting ${nickname} also deletes ${sessions} you logged on it${laps}, with their setups, notes and outcomes${baseline}. This cannot be undone.`;
}
