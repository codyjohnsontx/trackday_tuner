 ...
 * - no retained rider question text has outlived its 90 days
 *   (`ai_text_retention`), which is the only proof the purge job runs
 *
...
export async function checkAiTextRetention(now: Date = new Date()): Promise<HealthCheck> {
  return timed('ai_text_retention', async () => {
    ...
    if (count === null) {
      ...
      const err = new Error('ai_request_text answered without a count.');
      err.name = 'MissingCountError';
      throw err;
    }
    if (count > 0) {
      const err = new Error(`${count} ai_request_text rows are past retain_until by more than 36 hours.`);
      err.name = `OverdueRetainedTextError:${count}`;
      throw err;
    }
    return '0 overdue';
  });
