 ...
 * after 90 days, and a keeping rider's preview outlives a text row whose
 * insert failed, so it is counted too. So is the keep rule: a rider's text is
 * kept only once they have seen the notice and while their consent stands,
 * the routes and the database write a preview only for a rider who is keeping,
 * and the purge nulls any other daily - so a preview for a rider whose text
 * may not be kept, older than the same 36-hour grace, means the purge is not
 * keeping that rule either. Those rows are read through the `ai_requests_unretainable_previews`
