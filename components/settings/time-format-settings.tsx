  ...
  return (
    <div className="rounded-card bg-surface p-4">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div role="group" aria-label="Time format" className="flex overflow-hidden rounded-row border border-white/10 text-xs">
          <button
            type="button"
            aria-pressed={format === '12h'}
            onClick={() => set('12h')}
            className={cn(
              'rounded-row px-3 py-1.5 font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal/80',
              format === '12h' ? 'bg-ink text-canvas' : 'bg-surface text-ink-dim hover:text-ink',
            )}
          >
            12-hour
          </button>
          <button
            ...
            className={cn(
              'rounded-row px-3 py-1.5 font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal/80',
              format === '24h' ? 'bg-ink text-canvas' : 'bg-surface text-ink-dim hover:text-ink',
            )}
          >
            24-hour
          </button>
