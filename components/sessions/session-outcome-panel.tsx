    ...
    <section className="space-y-4 rounded-card bg-surface p-4">
      ...
      {!editing ? (
        ...
        <>
          ...
          <div className="grid grid-cols-4 gap-2 rounded-row bg-surface-2 p-1">{([['better','Better'],['same','Same'],['worse','Worse'],['unknown','Unsure']] as const).map(([value,label]) => <button key={value} type="button" aria-pressed={outcome === value} onClick={() => setOutcome(value)} className={cn('min-h-11 rounded-row px-1 text-xs font-semibold focus-visible:ring-2 focus-visible:ring-signal/80', outcome === value ? 'bg-ink text-canvas' : 'text-ink-dim')}>{label}</button>)}</div>
