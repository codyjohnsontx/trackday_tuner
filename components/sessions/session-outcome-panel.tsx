    ...
    <section className="space-y-4 rounded-card bg-surface p-4">
      ...
      {!editing ? (
        ...
        <>
          ...
          <label className="block space-y-1"><span className="text-xs text-ink-dim">AI recommendation tested (optional)</span><select value={recommendationId} onChange={(event) => setRecommendationId(event.target.value)} className="min-h-11 w-full rounded-row bg-surface-3 px-3 text-sm focus-visible:ring-2 focus-visible:ring-signal/80"><option value="">None — manual or coach-led change</option>{recommendations.map((recommendation) => <option key={recommendation.id} value={recommendation.id}>{recommendation.component ? formatComponentLabel(recommendation.component) : 'Setup'} · {recommendation.direction ?? recommendation.summary}</option>)}</select></label>
