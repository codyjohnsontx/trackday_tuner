...
export function SessionOutcomePanel({
  ...
  return (
    <section className="space-y-4 rounded-card border border-signal/30 bg-surface p-4">
      ...
      {!editing ? (
        <div><p className="text-lg font-semibold capitalize text-ink">{outcome === 'unknown' ? 'Unsure' : outcome}</p>{message ? <p className="mt-1 text-sm text-faster">{message}</p> : null}</div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-4 gap-2 rounded-row bg-canvas p-1">{([['better','Better'],['same','Same'],['worse','Worse'],['unknown','Unsure']] as const).map(([value,label]) => <button key={value} type="button" aria-pressed={outcome === value} onClick={() => setOutcome(value)} className={cn('min-h-11 rounded-row px-1 text-xs font-semibold focus-visible:ring-2 focus-visible:ring-signal/80', outcome === value ? 'bg-ink text-canvas' : 'text-ink-dim')}>{label}</button>)}</div>
          <label className="block space-y-1"><span className="text-xs text-ink-dim">AI recommendation tested (optional)</span><select value={recommendationId} onChange={(event) => setRecommendationId(event.target.value)} className="min-h-11 w-full rounded-row bg-surface-3 px-3 text-sm focus-visible:ring-2 focus-visible:ring-signal/80"><option value="">None — manual or coach-led change</option>{recommendations.map((recommendation) => <option key={recommendation.id} value={recommendation.id}>{recommendation.component ?? 'Setup'} · {recommendation.direction ?? recommendation.summary}</option>)}</select></label>
