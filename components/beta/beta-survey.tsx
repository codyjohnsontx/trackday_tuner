...
export function BetaSurvey() {
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-cyan-400/20 bg-zinc-900/70 p-4">
      ...
      <label className="block space-y-1"><span className="text-xs text-zinc-400">What is the biggest problem we should solve next?</span><textarea value={problem} onChange={(event) => setProblem(event.target.value)} maxLength={1000} rows={3} className="w-full rounded-xl border border-zinc-700 bg-zinc-950 px-3 py-3 text-sm focus-visible:ring-2 focus-visible:ring-cyan-400/80" /></label>
      <label className="flex items-start gap-3 text-xs text-zinc-400"><input type="checkbox" checked={interview} onChange={(event) => setInterview(event.target.checked)} className="mt-0.5 h-5 w-5 accent-cyan-400 focus-visible:ring-2 focus-visible:ring-cyan-400/80" /><span>I&apos;m open to a 20-minute rider interview.</span></label>
